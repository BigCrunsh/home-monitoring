...
# Mark all tests in this module as requiring InfluxDB
...
async def test_electricity_prices_available(influxdb_repo: InfluxDBRepository):
    ...
    """
    query = "SELECT * FROM electricity_prices_euro " "ORDER BY time DESC LIMIT 1"
...
async def test_electricity_consumption_periods_available(
    ...
    assert not missing_periods, (
        f"Missing consumption periods: {missing_periods}. " f"Found: {found_periods}"
    )
...
async def test_electricity_costs_periods_available(
    ...
    missing_periods = [p for p in expected_periods if p not in found_periods]
    assert not missing_periods, (
        f"Missing cost periods: {missing_periods}. " f"Found: {found_periods}"
    )
...
async def test_data_freshness(influxdb_repo: InfluxDBRepository):
    ...
    price_query = "SELECT * FROM electricity_prices_euro ORDER BY time DESC LIMIT 1"
    price_result = [row async for row in influxdb_repo.query(price_query)]
    ...
    consumption_result = [row async for row in influxdb_repo.query(consumption_query)]
...
async def test_iobroker_query_format(influxdb_repo: InfluxDBRepository):
    ...
    price_query_alt = "SELECT * FROM electricity_prices_euro ORDER BY time DESC LIMIT 1"

    result = [row async for row in influxdb_repo.query(price_query_alt)]
    ...
    result = [row async for row in influxdb_repo.query(consumption_query)]
    ...
    result = [row async for row in influxdb_repo.query(stats_query)]
