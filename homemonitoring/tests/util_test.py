...
class TestUtils(TestCase):
    ...
    def test_get_date_ranges(self):
        ...
        start_datetime = datetime.datetime(2020, 4, 11, 11, 58, 10, 20)
        end_datetime = datetime.datetime(2020, 4, 11, 11, 59, 10, 20)
        ...
        # end is new quarter
        end_datetime = datetime.datetime(2020, 4, 11, 12, 0, 0, 0)
        s, e = get_date_ranges('Europe/Berlin', start_datetime, end_datetime)
        np.testing.assert_equal(s, np.array([end_datetime]))
        np.testing.assert_equal(e, np.array([end_datetime]))

        # dist between start and end is more than a month (has to be split)
        end_datetime = datetime.datetime(2020, 5, 11, 12, 0, 0, 0)
        s, e = get_date_ranges('Europe/Berlin', start_datetime, end_datetime)
        np.testing.assert_equal(s, np.array([
